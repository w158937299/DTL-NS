...
if args.ps == 'enhance':
    train_cf = read_cf_us(directory + 'train_%s.txt' % (args.ps_param))
else:
    train_cf = read_cf_us(directory + 'train.txt')

test_cf = read_cf_us(directory + 'test.txt')
if args.dataset not in have_val:
    valid_cf = test_cf.copy()
else:
    valid_cf = read_cf_us(directory + 'valid.txt')
...
def valid_one_user(x):
    ...
    # user u's items in the test set
    user_pos_test = valid_user_set[u]
